...
STDMETHODIMP OS12::QueryInterface(REFIID riid, void** ppv)
{
	...
	*ppv = NULL;
	...
	else rc = E_NOINTERFACE;	//запрошенный интерфейс не поддерживается текущим объектом.

	if (rc == S_OK) this->AddRef();	// увеличить счетчик ссылок объекта.
	LOG("OS12::QueryInterface rc = ", rc);
	return rc;
};

// реализацию методов интерфейсов IAdder и IMultiplier 
//увеличивает счетчик ссылок на объект и возвращает новое значение счетчика ссылок.
...
//STDMETHODIMP для определения методов интерфейсов COM.  для определения методов, возвращающих HRESULT (код результата).

//STDMETHODIMP_(тип) : Этот макрос также используется для определения методов интерфейсов COM, но позволяет указать тип возвращаемого значения метода

STDMETHODIMP OS12::Add(const double lOp1, const double lOp2, double* pResult)
{
	*pResult = lOp1 + lOp2;	//возвращает результат через указатель pResult
	return S_OK;
}
