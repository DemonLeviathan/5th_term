...
OS12HANDEL OS12::Init() {
	...
	HRESULT hr0 = CoCreateInstance(CLSID_CA, NULL, CLSCTX_INPROC_SERVER, IID_IUnknown, (void**)&pIUnknown);//создает экземпляр COM-объекта
	if (SUCCEEDED(hr0))
	{
		return pIUnknown;
	}
...
void OS12::Dispose(OS12HANDEL h) {
	((IUnknown*)h)->Release();	// Освобождение интерфейса IUnknown объекта
	CoFreeUnusedLibraries();	// Завершение работы с библиотекой COM
}
