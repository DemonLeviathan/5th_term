...
namespace Lab11_Framework.Pages
{
    public class MainPage : DriverInfo.DriverInfo
    {
        ...
        private readonly By GlobusLocator = By.CssSelector("#menu-langauges > li:nth-child(1) > a > i");

        private readonly By GermanyLanguage =
            By.CssSelector("By.CssSelector(\"#menu-langauges [href='/eng/switchlanguage/to/ger/179']\")");


        public MainPage ChangeLamguage()
        {
            Driver.FindElement(GlobusLocator);
            // Находим меню
            IWebElement languageMenu = Driver.FindElement(GlobusLocator);

            // Используем Actions для наведения мыши на меню
            Actions actions = new Actions(Driver);
            actions.MoveToElement(languageMenu).Perform();
            Thread.Sleep(2000);
            // Находим элемент в выпадающем меню и кликаем на него
            IWebElement germanLanguageLink = Driver.FindElement(GermanyLanguage);
            germanLanguageLink.Click();
                foreach (string handle in Driver.WindowHandles)
                {
                    Driver.SwitchTo().Window(handle);
                    string currentUrl = Driver.Url;
                    if (currentUrl == "https://www.bvb.de/ger/News")
                    {
